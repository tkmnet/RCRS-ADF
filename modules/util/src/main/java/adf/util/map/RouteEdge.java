...
public class RouteEdge {
    ...
    private RouteEdge(WorldData world, List<EntityID> path) {
    ...
    public static RouteEdge getInstance(WorldData world, List<EntityID> path) {
    ...
    private void initRoadDistance(WorldData world) {
        this.roadDistance = new HashMap<>();
        int limit = this.element.size() - 1;
        Area area = (Area)world.getEntity(this.firstNodeID);
        this.roadDistance.put(this.firstNodeID, DistanceUtil.getDistance(world.getLocation(area), PositionUtil.getEdgePoint(area.getEdgeTo(this.element.get(1)))));
        area = (Area)world.getEntity(this.secondNodeID);
        this.roadDistance.put(this.secondNodeID, DistanceUtil.getDistance(world.getLocation(area), PositionUtil.getEdgePoint(area.getEdgeTo(this.element.get(limit - 1)))));
        ...
        for(int i = 1; i < limit; i++) {
            ...
            double distance = DistanceUtil.getDistance(area.getEdgeTo(this.element.get(i - 1)), area.getEdgeTo(this.element.get(i + 1)));
