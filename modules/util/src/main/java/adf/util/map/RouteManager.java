...
public class RouteManager {
    ...
    public RouteManager(WorldData world) {
    ...
    public void setPassable(WorldData world, EntityID areaID, boolean flag) {
    ...
    private void registerPassable(WorldData world, List<EntityID> path) {
    ...
    // remove node
    private void removeNode(WorldData world, EntityID areaID) {
    ...
    // remove edge
    private void removeEdge(WorldData world, EntityID areaID) {
        if(this.impassableArea.contains(areaID)) {
            return;
        }
        this.impassableArea.add(areaID);
    ...
    // add node
    private void addNode(WorldData world, EntityID areaID) {
        if(!this.impassableArea.contains(areaID)) {
            return;
        }
    ...
    // add edge
    private void addEdge(WorldData world, EntityID areaID) {
        if(!this.impassableArea.contains(areaID)) {
            return;
        }
        this.impassableArea.remove(areaID);
    ...
    private void analysis(WorldData world) {
    ...
    private void analysisRoad(WorldData world, Collection<StandardEntity> roads, Map<EntityID, Set<EntityID>> processedNeighbours) {
    ...
    private Set<StandardEntity> analysisBuilding(WorldData world, Map<EntityID, Set<EntityID>> processedNeighbours) {
        //edge road only
        Set<StandardEntity> processedRoad = new HashSet<>();
        //Buildings
    ...
    private void register(WorldData world, List<EntityID> path) {
        RouteEdge edge = RouteEdge.getInstance(world, path);
        if(edge != null) {
            int size = path.size() - 1;
            for (int i = 1; i < size; i++) {
